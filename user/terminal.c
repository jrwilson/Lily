...
#define SCAN_CODES_OUT_NO 4
#define MOUSE_PACKETS_OUT_NO 5
#define STDIN_NO 6
#define VGA_OP_NO 7
#define DESTROYED_NO 8
...
BEGIN_OUTPUT (NO_PARAMETER, VGA_OP_NO, "vga_op", "vga_op_list", vga_op, ano_t ano, int param)
{
  ...
  if (vga_op_precondition ()) {
    ...
    if (active_client->graphics_refresh) {
      /* Send the data. */
      if (vga_op_list_write_bassign (&output_buffer, VGA_TEXT_MEMORY_BEGIN, PAGE_LIMIT_POSITION * LINE_LIMIT_POSITION * CELL_SIZE, active_client->bd) != 0) {
...
/*  *\/ */
/* BEGIN_INPUT (NO_PARAMETER, SCAN_CODE_NO, "scan_code", "buffer_file_t", scan_code, ano_t ano, int param, bd_t bda, bd_t bdb) */
/* { */
/*   initialize (); */
...
/* BEGIN_OUTPUT (NO_PARAMETER, TEXT_NO, "text", "buffer_file_t", text, ano_t ano, int param) */
/* { */
/*   initialize (); */
