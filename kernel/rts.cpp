...
namespace rts {
  ...
  //
  //   buffer, buffer_size, and flag can be used to manipulate buffers.
  ...
  //   The buffer_size is the minimum of the specified size and known physical capacity.
	  ...
	  int flags)
  {
	...
	if (b != 0) {
	  // Buffer exists.
	  // Correct the size.
	  buffer_size = min (buffer_size, b->capacity ());
	}
	else {
    ...
    scheduler::finish (bd, buffer_size, flags);
  }
