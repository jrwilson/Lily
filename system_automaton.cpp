...
namespace system_automaton {
  ...
  create_action_test ()
  {
    ...
    kputs ("creating input_automaton\n");
    {    
	...
	vm_manager::switch_to_directory (vm_manager::page_directory_physical_address ());
	
	vm_manager::map (pd, frame, paging_constants::SUPERVISOR, paging_constants::WRITABLE);
	for (size_t i = 0; i < PAGE_ENTRY_COUNT; ++i) {
	  kassert (pd->entry[i].frame_ == i);
	}
	vm_manager::unmap (pd);
    ...
    // kputs ("creating output_automaton\n");
    // {
    //   // First, create a new page directory.
